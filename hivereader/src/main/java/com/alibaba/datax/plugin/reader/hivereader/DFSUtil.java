...
public class DFSUtil {
    ...
    public DFSUtil(Configuration taskConfig) {
        ...
        if (null != hadoopSiteParams) {
            ...
            hadoopConf.addResource("/opt/idp/backend/conf/core-site.xml");
            hadoopConf.addResource("/opt/idp/backend/conf/hdfs-site.xml");
        }
    ...
    private enum Type {
        STRING, LONG, BOOLEAN, DOUBLE, DATE, TINYINT
    }
    ...
    public boolean checkHdfsFileType(String filepath, String specifiedFileType) {
        ...
        try {
            ...
            //这里解决hive创建临时表文件大小为0  datax检测文件类型错误的问题 暂时先这么做
            {
                ...
                LOG.info("file path >> {}, file readSize >> {}", filepath, readSize);
                if (filepath.contains("hive") && readSize == 0) {
                    return true;
                }
            }
